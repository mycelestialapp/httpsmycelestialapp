...
const GLOW_CSS = (hex: string) => `0 0 8px ${hex}, 0 0 16px ${hex}`;
...
function PillarChar({ char, interactive }: { char: string; interactive?: boolean }) {
  ...
  return (
    <span
      ...
      style={{ color, textShadow: GLOW_CSS(color), fontWeight: 700, fontSize: 20 }}
    >
      {char}
    </span>
  );
...
function BaziChartImageInner({ info, baziResult, premium = false, showQrCaption = true }: BaziChartImageProps) {
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, fontStyle: 'italic', fontFamily: C.font.title, letterSpacing: 4, color: C.duanyu.text_color, margin: 0 }}>
            天机洞察 · 四柱命盘
          </h1>
        </div>
        ...
        <div style={{ marginBottom: 24, paddingBottom: 16, borderBottom: `1px solid ${C.qrcode.color}` }}>
          ...
          <div style={{ fontSize: 14, color: '#c0c0c0', marginTop: 6 }}>{birthStr} {info.region ? ` · ${info.region}` : ''}</div>
        </div>

        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          {pillars.map(({ label, value }) => (
            <div key={label} className="bazi-pillar-column" style={{ width: 84, minWidth: 84 }}>
        ...
        <div style={{ marginBottom: 20, fontSize: C.font.content_size }}>
          ...
          {baziResult.dayMaster && (
            <div style={{ paddingLeft: C.columns.elements.hidden_stems.indent, marginBottom: 6 }}>
              ...
              <span style={{ color: getCharColor(baziResult.dayMaster), textShadow: GLOW_CSS(getCharColor(baziResult.dayMaster)), fontWeight: 700 }}>{baziResult.dayMaster}</span>
            </div>
          )}
          ...
          <div style={{ paddingLeft: C.columns.elements.na_yin.indent, color: C.columns.elements.na_yin.color, fontSize: 11, background: 'linear-gradient(90deg, transparent, rgba(230,194,0,0.1))', padding: '8px 0' }}>
            <span style={{ marginRight: 8 }}>纳音</span>
            {baziResult.nayin || '—'}
          </div>
        </div>

        <div style={{ marginTop: C.duanyu.margin_top, borderTop: C.duanyu.border_top, padding: C.duanyu.padding, lineHeight: C.duanyu.line_height, textIndent: C.duanyu.first_line_indent, color: C.duanyu.text_color, fontFamily: C.font.content, fontSize: 14, fontWeight: 300 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: C.qrcode.margin_top, paddingTop: 20, borderTop: `1px solid ${C.qrcode.color}` }}>
          <div className="bazi-qr-wrap" style={{ border: C.qrcode.border, borderRadius: 12, padding: 8, display: 'inline-block' }}>
            <QRCodeSVG value={C.qrcode.url} size={88} level="H" includeMargin={false} fgColor={C.qrcode.color} bgColor={C.qrcode.bg_color} />
          </div>
          {showQrCaption && (
            <div style={{ marginTop: 12, fontSize: 13, fontWeight: C.qrcode.text_weight, color: C.qrcode.text_color, fontFamily: C.qrcode.text_font, textAlign: 'center', whiteSpace: 'pre-line' }}>
              {C.qrcode.text}
            </div>
          )}
        </div>

        <div aria-hidden className="bazi-watermark" style={{ position: 'absolute', right: 20, bottom: 16, fontSize: C.watermark.font_size, color: C.watermark.color, fontFamily: C.watermark.font, opacity: C.watermark.opacity, transform: `rotate(${C.watermark.angle}deg)`, pointerEvents: 'none' }}>
          {C.watermark.text}
        </div>
      </div>
    </div>
